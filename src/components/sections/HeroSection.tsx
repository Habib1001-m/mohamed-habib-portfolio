...
export default function HeroSection({ lang }: HeroSectionProps) {
  ...
  const shellLabel = isRtl ? "واجهة تنفيذية" : "Executive interface";
  const orbitLabels = isRtl ? ["أنظمة", "خصوصية", "ذكاء"] : ["Systems", "Privacy", "AI"];

  return (
    <header id="hero-section" className="min-h-screen relative flex items-center pt-24 pb-16 overflow-hidden">
      <div className="pointer-events-none absolute inset-x-6 top-28 h-px bg-gradient-to-r from-transparent via-orange-400/25 to-transparent" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_22%,rgba(249,115,22,0.14),transparent_28%),radial-gradient(circle_at_82%_28%,rgba(245,158,11,0.12),transparent_30%),radial-gradient(circle_at_52%_78%,rgba(127,29,29,0.12),transparent_34%)]" />
      <div className="pointer-events-none absolute left-1/2 top-1/2 h-[34rem] w-[34rem] -translate-x-1/2 -translate-y-1/2 rounded-full border border-orange-400/10" />
      <div className="max-w-7xl mx-auto px-6 w-full grid lg:grid-cols-12 gap-14 xl:gap-20 items-center relative z-10">
        <div className={`lg:col-span-7 flex flex-col justify-center text-center ${isRtl ? "lg:text-right font-arabic" : "lg:text-left"}`}>
          <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 bg-orange-500/[0.08] border border-orange-500/20 text-orange-300 rounded-full w-fit mb-6 mx-auto shadow-[0_0_45px_rgba(249,115,22,0.12)] ${isRtl ? "lg:mx-0 lg:mr-0" : "lg:mx-0"}`}>
          ...
          <div className={`mb-4 text-[10px] text-orange-100/55 ${isRtl ? "font-arabic" : "font-mono uppercase tracking-[0.34em]"}`}>
            {shellLabel}
          </div>

          <h1 className={`display-text max-w-[760px] mx-auto ${isRtl ? "lg:mr-0 lg:ml-auto" : "lg:ml-0 lg:mr-auto"} text-[2.85rem] sm:text-[4rem] md:text-[4.8rem] xl:text-[5.7rem] text-white drop-shadow-[0_0_26px_rgba(249,115,22,0.12)]`}>
            <span className="block text-slate-50">{h.title1[lang]}</span>
            <span className="block bg-gradient-to-r from-[var(--habib-orange)] via-orange-200 to-[var(--habib-gold)] bg-clip-text text-transparent py-1">
              {h.title2[lang]}
            </span>
          ...
          <div className={`flex flex-wrap items-center gap-4 justify-center ${isRtl ? "lg:justify-end" : "lg:justify-start"} mb-10`}>
            <a
              ...
              className={`ds-action ds-action-primary px-6 shadow-[0_0_32px_rgba(249,115,22,0.18)] ${actionTextClass}`}
            >
          ...
          <div className={`w-full max-w-2xl mx-auto lg:mx-0 rounded-[var(--habib-radius-xl)] border border-white/10 bg-black/35 p-2.5 shadow-2xl shadow-black/20 backdrop-blur ${isRtl ? "font-arabic" : ""}`}>
        ...
        <div className="lg:col-span-5 flex justify-center">
          <div className="relative">
            <div className="pointer-events-none absolute inset-[-2.4rem] rounded-full border border-orange-400/10" />
            <div className="pointer-events-none absolute inset-[-1.2rem] rounded-full border border-amber-300/10" />
            <div className={`pointer-events-none absolute ${isRtl ? "right-[-1.8rem]" : "left-[-1.8rem]"} top-10 z-20 rounded-full border border-white/10 bg-black/45 px-3 py-1 text-[10px] text-orange-100/70 backdrop-blur ${isRtl ? "font-arabic" : "font-mono uppercase tracking-[0.18em]"}`}>
              {orbitLabels[0]}
            </div>
            <div className={`pointer-events-none absolute ${isRtl ? "left-[-2rem]" : "right-[-2rem]"} top-1/3 z-20 rounded-full border border-white/10 bg-black/45 px-3 py-1 text-[10px] text-amber-100/70 backdrop-blur ${isRtl ? "font-arabic" : "font-mono uppercase tracking-[0.18em]"}`}>
              {orbitLabels[1]}
            </div>
            <div className={`pointer-events-none absolute bottom-12 left-1/2 z-20 -translate-x-1/2 rounded-full border border-white/10 bg-black/45 px-3 py-1 text-[10px] text-yellow-100/70 backdrop-blur ${isRtl ? "font-arabic" : "font-mono uppercase tracking-[0.18em]"}`}>
              {orbitLabels[2]}
            </div>
            <button
              type="button"
            ...
            >
              <div className={`absolute inset-[-3px] rounded-full bg-gradient-to-r from-orange-600 via-amber-400 to-red-500 blur-md transition-opacity duration-500 ${isPortraitActive ? "opacity-70" : "opacity-50 group-hover:opacity-70"}`} />
              <div className="w-64 h-64 sm:w-72 sm:h-72 md:w-80 md:h-80 xl:w-[22rem] xl:h-[22rem] rounded-full overflow-hidden border-4 border-[var(--habib-bg)] relative z-10 bg-zinc-900 shadow-[0_28px_100px_rgba(0,0,0,0.55)]">
                <img
